...
use methods::{PWD_CHECKER_ID, PWD_CHEKER_ELF};
use risc0_zkvm::{
...
fn main() {
    ...
    // For example: let env =
    // ExecutorEnv::builder().add_input(&vec).build().unwrap();

    // Next, we make an executor, loading the (renamed) ELF binary.
    let mut exec = Executor::from_elf(env, PWD_CHEKER_ELF).unwrap();
    ...
    // verify your receipt
    receipt.verify(PWD_CHECKER_ID.into()).unwrap();
}
